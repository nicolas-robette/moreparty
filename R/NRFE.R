...
NRFE <- function(Y, X, ntree = 3000, measure = NULL, parallel = FALSE, ...) {
  ...
  for(i in 1:(ncol(X)-1)) selections[[i]] <- selections[[ncol(X)]][1:i]
  #errors <- c()
  ...
  foo <- function(i) {
    ...
    # if (recompute == T & i > 1) selections[[i - 1]] <- names(sort(vi_fct(forest), decreasing = T))[-i]
  }
  
  errors <- plyr::aaply(ncol(X):1, 1, .fun=foo, .parallel=parallel, .paropts=list(.packages="party"))
